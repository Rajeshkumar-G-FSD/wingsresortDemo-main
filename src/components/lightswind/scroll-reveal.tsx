...
export function ScrollReveal({
  ...
  // Split text into words and spaces, keeping each part identifiable for stable keys.
  const splitText = useMemo(() => {
    ...
    return text
      ...
      .map((part, index) => ({
        value: part,
        isSpace: Boolean(part.match(/^\s+$/) && part.length > 0),
        originalIndex: index,
      }))
  ...
  return (
    <motion.div ref={containerRef} style={{ rotate: rotation }} className={cn('my-5 transform-gpu', containerClassName)}>
      <motion.p
        className={cn('leading-relaxed font-semibold', sizeClasses[size], alignClasses[align], variantClasses[variant], textClassName)}
