...
export const RoomsPage: React.FC<RoomsPageProps> = ({ initialCheckIn, initialCheckOut, initialAdults, onBack }) => {
  ...
  return (
    <div className="animate-fadeIn bg-[#fbf9f6] text-[#1b1c1a]">
      {/* Hero */}
      <section className="relative flex min-h-[56vh] items-end overflow-hidden sm:min-h-[64vh]">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#00201f]/92 via-[#00201f]/50 to-[#00201f]/15" />
        ...
        <div className="mx-auto -mt-8 max-w-[1000px] rounded-3xl border border-[#e4e2df] bg-white p-3 shadow-xl sm:-mt-10">
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_1fr_1fr_auto]">
                ...
                <div className="absolute left-0 top-[calc(100%+8px)] z-40 w-full min-w-[180px] rounded-3xl border border-[#e4e2df] bg-white p-2 shadow-2xl animate-fadeIn">
                  {[1, 2, 3, 4, 6, 8, 10, 12].map((n) => (
                    <button
                      ...
                      className={`flex w-full items-center justify-between rounded-full px-4 py-2.5 text-xs font-semibold transition-colors ${adults === n ? 'coral-gradient text-white' : 'text-[#1b1c1a] hover:bg-[#f5f3f0]'}`}
                    >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-full bg-[#f06c52] px-6 py-3.5 text-[10px] font-bold uppercase tracking-[.12em] text-white transition hover:bg-[#de573e]"
            >
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {rankedRooms.map((room) => {
              ...
              return (
                <div
                  key={room.id}
                  className={`hover-lift flex flex-col overflow-hidden rounded-3xl border bg-white soft-shadow transition-colors ${
                    isSelected ? 'border-[#004449]' : 'border-[#e8e3dc]'
                  }`}
                >
                  <div className="relative aspect-[4/3] w-full overflow-hidden">
                    ...
                    <span className="absolute left-3 top-3 rounded-full bg-[#004449]/85 px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-white backdrop-blur-sm">
                      {room.badge}
                    </span>
                    {!fits && (
                      <span className="absolute right-3 top-3 rounded-full bg-white/90 px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-[#a93721]">
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    ...
                    <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1.5 text-xs text-[#3f4849]">
                      <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm text-[#f06c52]">square_foot</span>{room.sizeSqft} sq ft</span>
                      <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm text-[#f06c52]">group</span>Up to {room.maxAdults}</span>
                      <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm text-[#f06c52]">king_bed</span>{room.bedType}</span>
                    </div>

                    <div className="mt-4 flex flex-wrap gap-1.5">
                      {room.amenities.slice(0, 3).map((a) => (
                        <span key={a} className="rounded-full bg-[#eef3f2] px-2.5 py-1 text-[10px] font-semibold text-[#004449]">{a}</span>
                      ))}
                    </div>
                    ...
                    <div className="mt-4 flex gap-2">
                      <button
                        ...
                        className="flex-1 rounded-full border border-[#004449] px-4 py-2.5 text-xs font-semibold text-[#004449] transition-colors hover:bg-[#004449] hover:text-white"
                      >
                        View Details
                      </button>
                      <button
      ...
      {/* Detail spotlight */}
      {selectedRoom && (
        <section ref={detailRef} className="scroll-mt-20 border-t border-[#e8e3dc] bg-[#f5f3f0] px-5 py-16 md:px-12 md:py-20">
          <div className="mx-auto max-w-[1280px]">
                ...
                <div className="mt-3 flex gap-2 overflow-x-auto no-scrollbar">
                  {selectedRoom.gallery.map((img) => (
                    <button
                      ...
                      className={`h-16 w-20 shrink-0 overflow-hidden rounded-xl border-2 transition-colors ${activeImage === img ? 'border-[#f06c52]' : 'border-transparent'}`}
                    >
              ...
              <div>
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  <div className="rounded-xl bg-white p-3 text-center">
                    ...
                    <span className="mt-1 block text-xs font-semibold text-[#1b1c1a]">{selectedRoom.sizeSqft} sq ft</span>
                  </div>
                  <div className="rounded-xl bg-white p-3 text-center">
                    <span className="material-symbols-outlined text-lg text-[#004449]">group</span>
                    <span className="mt-1 block text-xs font-semibold text-[#1b1c1a]">Up to {selectedRoom.maxAdults}</span>
                  </div>
                  <div className="rounded-xl bg-white p-3 text-center">
                    <span className="material-symbols-outlined text-lg text-[#004449]">bathtub</span>
                    <span className="mt-1 block text-xs font-semibold text-[#1b1c1a]">{selectedRoom.bathrooms} Bath{selectedRoom.bathrooms > 1 ? 's' : ''}</span>
                  </div>
                  <div className="rounded-xl bg-white p-3 text-center">
                    <span className="material-symbols-outlined text-lg text-[#004449]">home_work</span>
                    <span className="mt-1 block text-xs font-semibold text-[#1b1c1a]">{selectedRoom.roomCount} Available</span>
                  </div>
                </div>

                <div className="mt-5 flex flex-wrap gap-2">
                  {selectedRoom.amenities.map((a) => (
                    <span key={a} className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-[#004449]">{a}</span>
                  ))}
                </div>

                {/* Pricing table */}
                <div className="mt-6 overflow-hidden rounded-2xl border border-[#e4e2df] bg-white">
                ...
                <ul className="mt-5 space-y-2 text-xs text-[#3f4849]">
                  <li className="flex items-start gap-2"><span className="material-symbols-outlined mt-0.5 text-sm text-[#f06c52]">bed</span>Extra bed available for {formatINR(EXTRA_BED_CHARGE_PER_NIGHT)} / night.</li>
                  <li className="flex items-start gap-2"><span className="material-symbols-outlined mt-0.5 text-sm text-[#f06c52]">child_care</span>Children below {CHILD_FREE_AGE} years stay free.</li>
                  <li className="flex items-start gap-2"><span className="material-symbols-outlined mt-0.5 text-sm text-[#f06c52]">event_available</span>Free cancellation up to {CANCELLATION_DAYS_BEFORE_CHECKIN} days before check-in.</li>
                </ul>

                <button
                  onClick={() => setBookingRoom(selectedRoom)}
                  className="mt-6 w-full rounded-full bg-[#004449] py-4 text-xs font-bold uppercase tracking-widest text-white shadow-lg transition hover:bg-[#0e5d63] sm:w-auto sm:px-10"
                >
                  Book This Room
                </button>
              </div>
